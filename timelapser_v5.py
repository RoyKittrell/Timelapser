...
def main():
    ...
    run_dir = Path.cwd() / f"{stamp}_{args.mode}_v5_wifi"
    ...
    manifest = {
        "version": "5.0.9-autonomous-seed-bootstrap-nonstarving-ai-verified-exposure",
        "mode": args.mode,
        "run_dir": str(run_dir),
    ...
    logger.human("=" * 68)
    logger.human(" TIMELAPSER V5.0.9 — AUTONOMOUS SD BOOTSTRAP / NON-STARVING AI / VERIFIED EXPOSURE")
    logger.human("=" * 68)
    ...
    logger.human("Exposure control:     rec mode -> set_camprop (instant direct writes)")
    ...
    logger.human("AI authority:         async advice; ready exposure actions serviced before capture")
    logger.human("AI context:           measured 1m/5m brightness trend; mode label is not a trend assumption")
    logger.human(
        f"Lens profile:         {LENS_PROFILE_NAME}; physical widest f/{LENS_MIN_APERTURE:g}; "
        f"control floor f/{MIN_APERTURE:g}"
    )
    logger.human(f"Exposure preference:  prefer ISO before shutter exceeds {PREFERRED_MAX_SHUTTER_SECONDS:.3g}s")
    ...
    try:
        ...
        apply_baseline(camera, state, args.mode, logger, override=baseline_override)
        commander.start()
        run_startup_scout(camera, commander, state, logger, paths, args.transfer_mode)
        ...
        while state.snapshot()["next_frame_no"] <= args.max_frames:
            ...
            # V5.0.8 control-priority rule: ready exposure work MUST NOT starve.
            # Drain the commander's currently-ready actions once every loop, even when
            # a frame is already due. A correct exposure is more important than preserving
            # a nominal 5 s cadence while shooting stale settings.
            execute_ai_actions(camera, commander, state, logger)
            ...
            # V5.0.8: deterministic safety net. This is independent of the AI queue.
            # If the previous three frames are essentially saturated, force a 2 EV
            # shutter reduction immediately, then refresh the state used by telemetry.
            apply_clipping_failsafe(camera, state, logger, scene_history)
            ...
            if snap["downloaded_frames"] == 1 or snap["downloaded_frames"] % AI_REVIEW_EVERY_N_DOWNLOADED_FRAMES == 0:
                commander.request_review(
                    snap,
                    str(local_jpg),
                    f"periodic_frame_{frame_no}",
                )
